...
func TransferBetweenDebitAccounts(externalID string, fromAccountID, toAccountID uuid.UUID, amount *big.Int, currency Currency) *Transaction {
	return NewTransaction(nil).
		...
		AddEntry(toAccountID, Debit, Debit, amount, currency).
		MustBuild()
}

// Deposit creates a transaction for depositing an amount into a credit account from a cash account with debit normal side.
func Deposit(externalID string, debitAccountID, toCreditAccountID uuid.UUID, amount *big.Int, currency Currency) *Transaction {
	return NewTransaction(nil).
		WithExternalID(externalID).
		AddEntry(debitAccountID, Debit, Debit, amount, currency).
		AddEntry(toCreditAccountID, Credit, Credit, amount, currency).
		MustBuild()
}

// DepositWithFee its like Deposit but takes part of the amount as fee which is transferred to a fee account.
func DepositWithFee(externalID string, debitAccountID, creditFeeAccountID, toCreditAccountID uuid.UUID, feeTax int8, amount *big.Int, currency Currency) *Transaction {
	depositFee := TakePercent(amount, feeTax)

	return NewTransaction(nil).
		WithExternalID(externalID).
		AddEntry(debitAccountID, Debit, Debit, amount, currency).
		AddEntry(creditFeeAccountID, Credit, Credit, depositFee, currency).
		AddEntry(toCreditAccountID, Credit, Credit, new(big.Int).Sub(amount, depositFee), currency).
		MustBuild()
}

// Withdraw creates a transaction for withdrawing an amount from a credit account and a cash account with debit normal side.
func Withdraw(externalID string, debitAccountID, fromCreditAccountID uuid.UUID, amount *big.Int, currency Currency) *Transaction {
	return NewTransaction(nil).
		WithExternalID(externalID).
		AddEntry(debitAccountID, Credit, Debit, amount, currency).
		AddEntry(fromCreditAccountID, Debit, Credit, amount, currency).
		MustBuild()
}

// WithdrawWithFee is the same as Withdraw but takes part of the amount as fee which is transferred to a fee account.
func WithdrawWithFee(externalID string, debitAccountID, creditFeeAccountID, fromCreditAccount uuid.UUID, feeTax int8, amount *big.Int, currency Currency) *Transaction {
	withdrawFee := TakePercent(amount, feeTax)

	return NewTransaction(nil).
		WithExternalID(externalID).
		AddEntry(debitAccountID, Credit, Debit, new(big.Int).Sub(amount, withdrawFee), currency).
		AddEntry(creditFeeAccountID, Credit, Credit, withdrawFee, currency).
		AddEntry(fromCreditAccount, Debit, Credit, amount, currency).
		MustBuild()
}
